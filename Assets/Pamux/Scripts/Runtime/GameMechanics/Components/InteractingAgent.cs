...
namespace Pamux.Lib.Unity.Commons
{
    public class InteractingAgent : MonoBehaviour, IInteractingAgent
    {
        ...
        private List<Interactable> GetReachableInteractables() {
            ...
            Debug.DrawRay(this.raycastOrigin.position, this.raycastOrigin.forward * this.pcc.ActiveLocomotionZoneParameters.MaxInteractionDistance, Color.red);

            if (!Physics.Raycast(
                this.raycastOrigin.position,
                this.raycastOrigin.forward,
                out RaycastHit hit,
                this.pcc.ActiveLocomotionZoneParameters.MaxInteractionDistance,
